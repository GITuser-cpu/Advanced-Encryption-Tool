#!/usr/bin/env python3
"""Network Scanner with port scanning and WiFi scanning capabilities."""
...
class NetworkSniffer:
    ...
    def __init__(self):
        ...
        self.scan_type = "port"
        self.custom_ports = (
            "21-23,25,53,80,110,135-139,443,445,1433-1434,3306,3389,5432,5900,"
    ...
    def set_scan_type(self, scan_type):
        """Set the scan type (port or wifi)."""
    ...
    def _start_port_scan(self, interface):
        """Start port scanning using nmap."""
        try:
    ...
    def _read_nmap_output(self):
        """Read and parse nmap XML output."""
        try:
            xml_output = ""
    ...
    def _read_wifi_output(self):
        """Read and parse WiFi scan output."""
        try:
    ...
    def _parse_nmap_xml(self, xml_output):
        """Parse nmap XML output and create packet objects."""
        try:
    ...
    def _parse_windows_wifi(self, output):
        """Parse Windows WiFi scan output."""
        try:
            networks = re.findall(
...
class NetworkSnifferUI:
    """Main UI class for the network scanner."""
    ...
    def show_packet_details(self, _event):
        ...
        packet_index = (
            int(self.packet_tree.item(item, "tags")[0])
            if self.packet_tree.item(item, "tags") else 0
        )
    ...
    def on_packet_captured(self, packet):
        ...
        self.packet_tree.insert(
            ...
            tags=(str(len(self.packet_data)-1),)
        )
        ...
        if self.scan_type_var.get() == "wifi" and packet.protocol == "WiFi":
            ...
            wifi_values = (
                ...
                packet.rssi,
                packet.channel if packet.channel else "N/A",
                packet.payload.split('MAC:')[1].strip()
                if 'MAC:' in packet.payload else "N/A"
            )
